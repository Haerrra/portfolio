...
CREATE OR REPLACE TABLE `team`.`tech`.pxa_ord_funnel_cart_base AS (
...
, depth_2 AS (
  ...
  WHERE dt >= '20251001'
    ...
    AND button_id = 'basket_btn' -- 장바구니 퍼널 기준
...
, depth_3 AS (
  SELECT
    ...
    -- , first_value(b.hit_time) over (partition by b.hash_id, b.goods_no  order by hit_time) AS hit_time
  FROM depth_2 A
  LEFT JOIN
  (SELECT * FROM lake.bigquery.ga4_log
  WHERE dt >= '20251001'
