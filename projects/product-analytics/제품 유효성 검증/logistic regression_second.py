...
df = spark.sql("""
    ...
    FROM (select
            ...
            , b.action_date
        from
        (select hash_id, first_ord_date from `team`.`tech`.onboard_first_order_1000 where first_ord_dt between '20240825' and '20240930') A
    ...
    LEFT JOIN (select
            ...
            where purchase_cnt = 2) B
    ON c.hash_id = b.hash_id
    WHERE c.first_ord_date <= a.date
    AND b.ord_date >= a.date
    )
        ...
        , codinote_prd_click
        , d7_retention_yn
        , if(sum(d7_ord_yn) > 0, 1, 0) AS d7_second_ord_yn
        FROM SECOND_RAW
        GROUP BY ALL
""")
...
y = pandas_df['d7_second_ord_yn']  # 종속변수
...
plt.ylabel('Second Order (1 or 0)')
plt.title(f'Logistic Regression: {data_name.title()} vs. Second Order') #그래프에 추가되는 독립 변수명으로 변경
plt.legend()
plt.grid()
