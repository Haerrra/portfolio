...
| 레이어 | 스키마 패턴 (가상) | 의미 |
|---|---|---|
| **Bronze (CDC 원본)** | `musinsa.*` | Aurora/MySQL CDC 원본 적재 |
| **Silver (정제)** | `lake.gold.*` | 정제·조인 완료된 레이크 테이블 |
...
| **Team (분석 결과물)** | `team.tech.*` | 타 팀/타 DAG 결과 마트 chain |
...
| 온보딩 (분석팀) | `dags/pxa/customer-engagement/` | `team.tech.onboard_registration` |
...
| 레이어 | 테이블 (가상) |
|---|---|
| Bronze (`musinsa.*`) | `onboarding.participation`, `saving_point.point_list`, `coupon.coupon_member` |
| Silver (`lake.gold.*`) | `purchase`, `user`, `user_partitioned` |
| Mart (`datamart.datamart.*`) | `goods`, `orders`, `users` |
| Team (cross-ref) | `team.tech.onboard_registration` → `team.tech.onboard_first_order` → `team.tech.onboard_firstbuy_raw` (자기 도메인 chain) |
