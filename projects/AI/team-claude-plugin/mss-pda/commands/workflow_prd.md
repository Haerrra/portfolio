...
0. 워크플로우 시작 기록 (필수, 다른 step보다 먼저 실행)
   ...
   - 효과: `~/.claude/mss-pda-telemetry/.workflow_id`, `.workflow_start_ts` 사이드카 파일 생성
...
7. Wiki에 PRD 결과 저장
   ...
   - Wiki 업로드 경로는 **환경변수로 사용자 PC별로 설정**됨 (없으면 사용자에게 한 번 묻고 진행):
     - `MSS_WIKI_SPACE_KEY` — Confluence space key (예: 본인 personal space `~yourname.lastname` 또는 팀 space)
     - `MSS_WIKI_PARENT_PAGE_ID` — 업로드 대상 parent page ID
   - 문서 제목 규칙 예시
     - [PRD] 기능명 변경 사항
   - 업로드 후 URL을 확보한다.
...
9. 만족도 피드백
   - `/mss-pda:feedback --ticket {ticket_id}` 호출
