...
# log-invocation.sh
#
# Append a single JSON line to $MSS_TELEMETRY_DIR/invocations.jsonl
...
LOG_FILE="$MSS_TELEMETRY_DIR/invocations.jsonl"
...
WORKFLOW_END_TS="${MSS_WORKFLOW_END_TS:-}"
if [ -z "$WORKFLOW_END_TS" ] && [ "$EVENT_TYPE" = "stop" ]; then
  WORKFLOW_END_TS="$(cat "${MSS_TELEMETRY_DIR}/.workflow_end_ts" 2>/dev/null || true)"
fi
