...
> - 점수가 비어있거나 1~10 범위를 벗어나면 `MSS_FB_SCORE=""`로 비워서 호출 (스크립트가 알아서 null 처리)
> - 코멘트의 PII(이메일, 전화, 토큰)는 스크립트가 자동 마스킹하므로 그대로 전달 OK
> - `MSS_WORKFLOW_ID`는 `~/.claude/mss-pda-telemetry/.workflow_id` 파일에서 자동 fallback되므로 직접 export할 필요 없음
...
- 데이터는 `~/.claude/mss-pda-telemetry/feedback.jsonl`에 append되며, 추후 `/mss-pda:metrics`에서 집계
- `MSS_TELEMETRY_ENABLED=false`인 경우 즉시 종료(no-op)
...
5. 점수에 따른 추가 질문 (코멘트 수집)
   - **⚠️ 어시스턴트(Claude) 지침**: 코멘트는 반드시 **사용자가 자유 텍스트로 직접 입력**하도록 받아야 함.
     ...
     - ✅ 자유 입력(plain text)으로만 받기. 사용자 본인이 직접 쓴 문장만 `MSS_FB_COMMENT`에 기록
     - 비어있거나 사용자가 스킵하면 `MSS_FB_COMMENT=""`로 전달
   - 질문 문구 (그대로 사용):
...
6. 입력값을 환경변수로 export한 뒤 `scripts/log-feedback.sh`를 호출하여 기록
   ...
   - Environment Variables
      * `MSS_FB_SCORE` — 1~10 정수 또는 빈 문자열
      * `MSS_FB_COMMENT` — 자유 텍스트 (PII 자동 마스킹)
      * `MSS_FB_COMMAND` — `workflow-analysis` / `workflow-pipeline` / `workflow-prd` / 빈 문자열
      * `MSS_FB_CONTEXT` — 필수, 소문자: analysis / experiment / dashboard / request / task / others
      * `MSS_FB_SUMMARY` — 자유 텍스트 (선택)
      * `MSS_FB_TICKET_ID` — `PXA-###` 또는 빈 문자열
   - 자동 fallback (export 불필요)
      * `MSS_WORKFLOW_ID` — `${MSS_TELEMETRY_DIR}/.workflow_id` 파일에서 자동 로드
      * `MSS_USER_ID` — 세션 환경변수에서 자동 로드
   - 호출 예시:
     ```bash
     export MSS_FB_SCORE="9"
     export MSS_FB_CONTEXT="analysis"
     export MSS_FB_COMMENT="SQL 자동 생성 정확도 좋음"
     export MSS_FB_COMMAND="workflow-analysis"
     export MSS_FB_TICKET_ID="PXA-541"
