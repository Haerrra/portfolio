...
# log-feedback.sh
#
# Append a satisfaction-feedback record to $ACME_TELEMETRY_DIR/feedback.jsonl
#
# Inputs (via environment variables, set by the calling /acme-pda:feedback flow):
#   ACME_FB_SCORE       1-10 integer, or empty/null to skip
#   ACME_FB_COMMENT     자유 텍스트 (선택)
#   ACME_FB_COMMAND     /acme-pda:workflow-analysis 등
#   ACME_FB_TICKET_ID   PXA-### (선택)
#   ACME_FB_TAGS        comma-separated, e.g., "accurate,fast" (선택)
...
LOG_FILE="$ACME_TELEMETRY_DIR/feedback.jsonl"
...
# ===== 입력값 =====
SCORE="${ACME_FB_SCORE:-}"
COMMENT="${ACME_FB_COMMENT:-}"
CMD="${ACME_FB_COMMAND:-}"
CONTEXT="${ACME_FB_CONTEXT:-}"
SUMMARY="${ACME_FB_SUMMARY:-}"
TICKET="${ACME_FB_TICKET_ID:-}"
TAGS_RAW="${ACME_FB_TAGS:-}"
...
echo "$NEW_WORKFLOW_ID" > "${ACME_TELEMETRY_DIR}/.workflow_id" 2>/dev/null
echo "$TS"              > "${ACME_TELEMETRY_DIR}/.workflow_start_ts" 2>/dev/null
rm -f "${ACME_TELEMETRY_DIR}/.workflow_end_ts" 2>/dev/null
