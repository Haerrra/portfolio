...
본 문서는 mss-pda 플러그인이 수집하는 텔레메트리 이벤트의 스키마를 정의한다.

## Storage Layout

### 실시간 Databricks 적재 (workflow_id 단위)

`/mss-pda:feedback` 호출 = 워크플로우 종료 시점에 **그 워크플로우의 events 전체**가 Databricks 테이블에 INSERT 된다.

```
[Workflow A 진행 중]
  hooks → invocations.jsonl, feedback.jsonl (로컬 append)
  ↓
[Workflow A 종료: /mss-pda:feedback 호출]
  log-feedback.sh
    ├─ feedback row append
    ├─ 사이드카 갱신 (새 workflow_id)
    └─ flush-to-databricks.py 백그라운드 spawn
        ├─ workflow_id=A 필터링
        ├─ INSERT INTO team.tech.pxa_ai_invocations_raw VALUES (...)
        ├─ INSERT INTO team.tech.pxa_ai_feedback_raw VALUES (...)
        └─ 성공 시 .flushed_invocations.txt / .flushed_feedback.txt 에 workflow_id append
```

| 로컬 파일 | Databricks 테이블 |
|-----------|-------------------|
| `invocations.jsonl` | `team.tech.pxa_ai_invocations_raw` |
| `feedback.jsonl` | `team.tech.pxa_ai_feedback_raw` |
...
```
$MSS_TELEMETRY_DIR/                        # default: ~/.claude/mss-pda-telemetry/
...
`MSS_TELEMETRY_DIR`이 없으면 `~/.claude/mss-pda-telemetry/`를 기본값으로 사용.
...
| `user_id`           | string  | yes      | `MSS_USER_ID` 환경변수 값 (예: `haera.kang`)                      |
...
| `command`           | string  | no       | 슬래시 커맨드명 (`/mss-pda:workflow-analysis` 등)                  |
...
```json
{"event_id":"3fa8...","timestamp":"2026-04-29T10:30:01+09:00","user_id":"haera.kang","session_id":"abc-123","event_type":"prompt_submit","workflow_id":"wf-9c1...","command":"/mss-pda:workflow-analysis","ticket_id":"PXA-541"}
{"event_id":"4fa8...","timestamp":"2026-04-29T10:30:15+09:00","user_id":"haera.kang","session_id":"abc-123","event_type":"tool_post","workflow_id":"wf-9c1...","step":"explore_data","tool":"mcp__databricks__execute_sql","status":"success","duration_ms":4210,"ticket_id":"PXA-541"}
```
...
`/mss-pda:feedback` 커맨드에서 `log-feedback.sh`가 append.

| field         | type             | required | description                                                       |
|---------------|------------------|----------|-------------------------------------------------------------------|
| `feedback_id` | string           | yes      | UUID v4                                                           |
| `timestamp`   | string           | yes      | ISO 8601 KST                                                      |
| `user_id`     | string           | yes      | `MSS_USER_ID`                                                     |
...
```json
{"feedback_id":"a1b2...","timestamp":"2026-04-29T10:36:00+09:00","user_id":"haera.kang","session_id":"abc-123","command":"workflow-analysis","workflow_id":"wf-9c1...","context":"analysis","summary":"20대 여성 사용자 구매 패턴 분석","ticket_id":"PXA-541","score":9,"comment":"SQL 자동 생성 정확도 좋음","tags":[]}
```
...
- 로컬 파일은 사용자 PC(`~/.claude/mss-pda-telemetry/`)에만 저장, durable backup 용도
