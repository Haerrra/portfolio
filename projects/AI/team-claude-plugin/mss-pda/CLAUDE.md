...
- **Target Schema**: team.tech.* for analytics tables
- **Source Schemas**: musinsa.* and datamart.* and datalake.

### Query Examples
#### Query 1 – Onboard Registration Table
```sql
INSERT INTO team.tech.onboard_registration
SELECT 
    uid,
    hash_id,
    gender,
...
```sql
CREATE OR REPLACE TABLE team.tech.onboard_first_order_1000 AS (
SELECT 
    a.uid,
    a.hash_id,
    a.gender,
...
```sql
CREATE OR REPLACE TABLE team.tech.onboard_firstbuy_raw AS (
SELECT 
    uid,
    hash_id,
    gender,
...
FROM (
    ...
    FROM (
        SELECT * 
        FROM team.tech.onboard_registration 
...
- PR Title & Description:
  ...
  - Description example:
      ...
      Source: team.tech.user_actions_by_position_in_slot
      Target: team.tech.page_metrics_daily
