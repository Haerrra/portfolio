...
Workflow 경계(/mss-pda:feedback 호출 시)에 닫힌 워크플로우의 events를
Databricks team.tech.pxa_ai_invocations_raw / team.tech.pxa_ai_feedback_raw 테이블로 적재한다.
...
Required env:
    MSS_DATABRICKS_HOST          예: https://musinsa-analysis-ws.cloud.databricks.com
    MSS_DATABRICKS_TOKEN         Bearer 토큰
    MSS_DATABRICKS_WAREHOUSE_ID  SQL warehouse ID

Optional env:
    MSS_TELEMETRY_DIR                  default ~/.claude/mss-pda-telemetry
    MSS_DATABRICKS_INVOCATIONS_TABLE   default team.tech.pxa_ai_invocations_raw
    MSS_DATABRICKS_FEEDBACK_TABLE      default team.tech.pxa_ai_feedback_raw
    MSS_FLUSH_ENABLED                  default true; "false"로 두면 skip
"""
...
TELEMETRY_DIR = Path(os.environ.get(
    "MSS_TELEMETRY_DIR",
    str(Path.home() / ".claude" / "mss-pda-telemetry"),
))
...
DBX_HOST = os.environ.get("MSS_DATABRICKS_HOST", "").rstrip("/")
DBX_TOKEN = os.environ.get("MSS_DATABRICKS_TOKEN", "")
WAREHOUSE_ID = os.environ.get("MSS_DATABRICKS_WAREHOUSE_ID", "")

INV_TABLE = os.environ.get(
    "MSS_DATABRICKS_INVOCATIONS_TABLE", "team.tech.pxa_ai_invocations_raw"
)
FB_TABLE = os.environ.get(
    "MSS_DATABRICKS_FEEDBACK_TABLE", "team.tech.pxa_ai_feedback_raw"
)
...
def main() -> int:
    ...
    if os.environ.get("MSS_FLUSH_ENABLED", "true").lower() != "true":
        return 0

    if not DBX_HOST or not DBX_TOKEN or not WAREHOUSE_ID:
        log_error(
            f"workflow={wf_id} skipped — "
            f"MSS_DATABRICKS_HOST / MSS_DATABRICKS_TOKEN / "
            f"MSS_DATABRICKS_WAREHOUSE_ID 중 하나 미설정"
        )
        return 1
